/**
 * BACnet MS/TP CRC calculations (from bacnet-stack, ASHRAE 135 Annex G).
 */
...
function calcHeaderCrc(headerBytes) {
  let crc = 0xff;
  for (const byte of headerBytes) {
    crc = HeaderCRC[crc ^ byte];
  }
  return (~crc) & 0xff;
}

function calcDataCrc(dataBytes) {
  let crc = 0xffff;
  for (const byte of dataBytes) {
    crc = ((crc >> 8) ^ DataCRC[(crc & 0xff) ^ byte]);
  }
  return (~crc) & 0xffff;
}

function verifyHeaderCrc(frameType, dest, src, lenMsb, lenLsb, headerCrc) {
  const expected = calcHeaderCrc([frameType, dest, src, lenMsb, lenLsb]);
  return expected === headerCrc;
}

function verifyDataCrc(dataBytes, crcLsb, crcMsb) {
  const expected = calcDataCrc(dataBytes);
  return expected === (crcLsb | (crcMsb << 8));
}
